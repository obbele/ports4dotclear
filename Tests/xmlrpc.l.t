...
tag0, tag1, tag2
Grüße Welt!.
This is a short content with unicode «←¢↓€→»
n
n
n
0
...
if( $output =~ /ID = (\d+)/){
	...
	$output = Pipe2::pipe2("$cmd --extract=$postid", '');
	like( $output, qr/Grüße Welt!/, "extract unicode excerpt");
	like( $output, qr/unicode «←¢↓€→»/, "extract unicode content");

	$output = Pipe2::pipe2("$cmd --show=$postid", '');
	like( $output, qr/Grüße Welt!/, "show unicode excerpt");
	like( $output, qr/unicode «←¢↓€→»/, "show unicode content");
...
if( $output =~ /ID = (\d+)/){
	...
	$output = Pipe2::pipe2("$cmd --extract=$postid", '');
	like( $output, qr/Grüße Welt/, "extract raw unicode excerpt");
	like( $output, qr/unicode «←¢↓€→»/, "extract unicode raw content");
